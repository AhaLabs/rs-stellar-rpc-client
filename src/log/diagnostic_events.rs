...
pub fn diagnostic_events(events: &Vec<String>, level: tracing::Level) {
    for (i, event_xdr) in events.iter().enumerate() {
        let event = DiagnosticEvent::from_xdr_base64(event_xdr, Limits::none()).expect("Failed to decode Diagnostic Event XDR");
